...
@retry(
    ...
    before_sleep=lambda retry_state: logger.info(f"Planner: Temporary error, retrying in {retry_state.next_action.sleep} seconds...")
)
async def run_orchestrator(job_id: str) -> None:
    ...
    try:
        ...
        # Create agent with tools and context (returns deployment name, tools, task, context)
        deployment_name, tools, task, context = create_agent(job_id, portfolio_summary, db)
