#!/usr/bin/env python3
"""
Test to verify LangFuse integration with Azure OpenAI
"""
...
async def test_langfuse_wrapper():
    """Test that LangFuse wrapper captures Azure OpenAI calls"""
    
    print("Testing LangFuse + Azure OpenAI Integration...")
    print("=" * 60)
    
    # Check configuration
    print(f"LANGFUSE_SECRET_KEY: {'✓ Set' if os.getenv('LANGFUSE_SECRET_KEY') else '✗ Missing'}")
    print(f"LANGFUSE_PUBLIC_KEY: {'✓ Set' if os.getenv('LANGFUSE_PUBLIC_KEY') else '✗ Missing'}")
    ...
    try:
        response = await client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "Say 'LangFuse integration working!' in one sentence."}
            ],
            temperature=0.7,
            max_tokens=50,
            # LangFuse-specific parameters (optional)
            name="test-trace",  # Name for the trace in LangFuse
            metadata={"test": "langfuse_integration", "agent": "tagger"}
        )
        ...
        print("You should see a trace named 'test-trace' with the API call details")
        print("=" * 60)
...
if __name__ == "__main__":
    asyncio.run(test_langfuse_wrapper())
